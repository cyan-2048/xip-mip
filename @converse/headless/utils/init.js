 ...
 */
import Storage from "@converse/skeletor/storage.js";
...
export function initPlugins(_converse) {
	...
	if (_converse.api.settings.get("singleton")) {
		["converse-bookmarks", "converse-controlbox", "converse-headline", "converse-register"].forEach(
			(name) => _converse.api.settings.get("blacklisted_plugins").push(name)
		);
	}
...
export async function attemptNonPreboundSession(credentials, automatic) {
	...
	if (api.settings.get("authentication") === LOGIN) {
		...
		if (!isTestEnv())
			log.debug("attemptNonPreboundSession: Couldn't find credentials to log in with");
	} else if (
